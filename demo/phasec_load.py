...
rbt_params_1 = np.loadtxt("output/phasec/h/best/gen10.txt")
# rbt_params_2 = np.load("output/cube6-1114/phaseb/gen11.npy")

robot1 = VariableRobot(x=3,y=3,z=2,disable=disable_param_332["h"])
...
sim = Simulator(k_ground=1, dt=1/1200, damping=0.9999,friction_s=0.8, friction_ground=0.8)
# for m in robot1.mass:
#     if not m.disable:
#         sim.mass.append(m)
#
...
#     sim.spring.append(s)
sim.reset(robot1.mass, robot1.spring)
...
def update():
    for i in range(timestep):
        for j, s in enumerate(robot1.spring):
            s.l0 = s.l00 * (1 + rbt_params_1[j,0] * np.sin(2*sim.t+rbt_params_1[j,1]))
